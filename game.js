...
let isPaused = false;
let enviando = false; // Añadida para evitar doble envío
...
// NUEVO: Evento de escritura en tiempo real (Sustituye al de Enter)
document.getElementById("inputWord").addEventListener("input", e => {
    ...
    if (input === word.toLowerCase()) {
        ...
        clearInterval(timerInterval);
        isPaused = true; // Pausa breve para feedback
        setTimeout(nextRound, 300); // 0.3 segundos para pasar a la siguiente
    }
});
...
function showScores() {
    ...
    // Limpiamos cualquier script de carga anterior para evitar conflictos
    const oldScript = document.getElementById('google-loader');
